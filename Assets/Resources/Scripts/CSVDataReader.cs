...
public class CSVDataReader : MonoBehaviour
{
    ...
    public List<string> GetPeopleAndObjectsInRange(int startTime, int endTime)
    {
        ...
        // Filtrer les �tudiants en fonction de l'heure
        foreach (var suspect in listOfSuspects)
        {
            Debug.Log("Checking " + suspect.getName());
            for (int time = startTime; time <= endTime; time++)
            {
                string location = suspect.queryTime(time);  // R�cup�re la localisation pour chaque heure
                if (location != "time not existant" && !results.Contains(suspect.getName()))
                {
                    results.Add(suspect.getName());  // Ajoute le nom de l'�tudiant si une pr�sence est trouv�e
                    break;  // Une fois trouv�, on arr�te de v�rifier pour ce suspect
                }
            }
        }
        ...
        {
            // V�rifiez si l'objet est dans la plage horaire et ajout du nom
    ...
    public void DisplayResults(int startTime, int endTime)
    {
        ...
        // Trouve le texte pour afficher les r�sultats
